...
Missile.prototype.SetRelativePosition = function(x,y,zoom){
    if(!this.isDone)
    {
        for(var i = 0; i < this.sprites.length; i++)
        {
            this.sprites[i].x = zoom * (this.x + x + this.subWidth/2);
            this.sprites[i].y = zoom * (this.y + y + this.subHeight/2);
            this.sprites[i].width = zoom * this.subWidth;
            this.sprites[i].height = zoom * this.subHeight;
        }
    }
...
Missile.prototype.Update = function()
{
    ...
    else
    {
        if(this.explosion == null)
        {
            ...
            this.explosion = new Explosion(this.layoutContainer,this.resource,this.layoutContext, this.target.x,this.target.y);
            this.isDone = true;
        }
        else
        {
