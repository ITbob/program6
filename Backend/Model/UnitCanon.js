...
UnitCanon.prototype.SetRelativePosition = function(x,y,zoom)
{
    for(var i = 0; i < this.firingSprites.length; i++)
    {
        this.firingSprites[i].x = zoom * (this.base.x + x + this.base.size/2);
        this.firingSprites[i].y = zoom * (this.base.y + y + this.base.size/2);
        this.firingSprites[i].width = zoom * this.base.size;
        this.firingSprites[i].height = zoom * this.base.size;
    }

    this.sprite.x = zoom * (this.base.x + x + this.base.size/2);
    this.sprite.y = zoom * (this.base.y + y + this.base.size/2);
    this.sprite.width = zoom * this.base.size;
    this.sprite.height = zoom * this.base.size;
};
