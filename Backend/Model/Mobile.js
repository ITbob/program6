...
Mobile.prototype.SetPosition = function(x,y){
	...
	for(var i = 0; i < this.sprites.length; i++)
	{
		...
		this.sprites[i].width = this.subsize;
		this.sprites[i].height = this.subsize;
	}
};
...
Mobile.prototype.Moving = function()
{
	...
	{
		this.CreateDust(this.GetCenter() + this.size/3*Math.cos(this.baseRadius), this.GetMiddle()+ this.size/3*Math.sin(this.baseRadius));
		this.CreateDust(this.GetCenter() - this.size/3*Math.cos(this.baseRadius), this.GetMiddle()- this.size/3*Math.sin(this.baseRadius));
	}
};

Mobile.prototype.CreateDust = function(x, y){
	var dust = new Dust(this.layoutContainer, this.resource, x, y);

	for (var i = 0; i < dust.sprites.length; i++) 
	{
		//console.log("dust " + i);
		this.layoutContainer.addChildAt(dust.sprites[i],this.layoutContainer.getChildIndex(this.sprites[0]));
	}
